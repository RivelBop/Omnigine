...
namespace Omni {
    ...
    public:
        ...
        bool update() {
            ...
            if      (t == IMAGE)   imageMap   .insert({asset, LoadImage(a)});
            else if (t == TEXTURE) textureMap .insert({asset, LoadTexture(a)});
            else if (t == FONT)    fontMap    .insert({asset, LoadFont(a)});
            else if (t == MODEL)   modelMap   .insert({asset, LoadModel(a)});
            else if (t == WAVE)    waveMap    .insert({asset, LoadWave(a)});
            else if (t == SOUND)   soundMap   .insert({asset, LoadSound(a)});
            else if (t == MUSIC)   musicMap   .insert({asset, LoadMusicStream(a)});
        ...
        void unloadAll() {
            while (!typeMap.empty()) unload(typeMap.begin()->first);
        }
